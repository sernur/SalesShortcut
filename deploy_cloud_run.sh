...
# Step 4.2: Deploy to Cloud Run (passing all service URLs)
...
gcloud run deploy $UI_CLIENT_SERVICE_NAME \
    ...
    --region $REGION \
    --allow-unauthenticated \
    --set-env-vars="GOOGLE_API_KEY=$GOOGLE_API_KEY,LEAD_FINDER_SERVICE_URL=$LEAD_FINDER_SERVICE_URL,LEAD_MANAGER_SERVICE_URL=$LEAD_MANAGER_SERVICE_URL,SDR_SERVICE_URL=$SDR_SERVICE_URL" \
    --project=$PROJECT_ID
